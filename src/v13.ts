 ...
 * BUILD & RUN:
 *   npm run build
 *   npx transcript-test . tests/transcripts/v13-custom-actions.transcript
...
class FamilyZooStory implements Story {
  ...
  initializeWorld(world: WorldModel): void {
    ...
    const stuffedAnimals = world.createEntity('stuffed animals', EntityType.OBJECT);
    ...
    const postcards = world.createEntity('postcards', EntityType.OBJECT);
    ...
    parrot.add(new IdentityTrait({ name: 'parrot', description: 'A magnificent scarlet macaw perched on a rope near the entrance. It tilts its head and watches you with one bright eye.', aliases: ['parrot', 'parrots', 'macaw', 'macaws', 'scarlet macaw', 'birds'], properName: false, article: 'a' }));
